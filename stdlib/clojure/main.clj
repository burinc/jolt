...
;; raised under :read-source is the reference's, on LispReader$ReaderException:
;; a read from a LineNumberingPushbackReader raises one around any error, and so
;; do jolt's own *in* readers (stdin, with-in-str), which number lines the same
;; way. The phase is the REPL's to add, not the reader's — read-string in a
;; program is that program's execution error on both runtimes — so the reader
;; stamps none and this seam, like load-string's, is where a read of source
